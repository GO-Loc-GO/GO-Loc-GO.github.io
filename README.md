# Xuanhan Zhu (朱轩汉)

Ph.D. Student
State Key Laboratory of Networking and Switching Technology
Beijing University of Posts and Telecommunications (BUPT)

**Email:** xuanhan.zhu@bupt.edu.cn

## About Me
Xuanhan Zhu received his B.Eng. degree in Internet of Things Engineering from Beijing University of Technology in 2023. He is currently pursuing his Ph.D. at the State Key Laboratory of Networking and Switching Technology, Beijing University of Posts and Telecommunications, under the supervision of Prof. Jinglin Li, Prof. Quan Yuan, and Prof. Guiyang Luo. 

His current research focuses on 3D computer vision and multi-agent collaborative perception for autonomous driving. He is particularly interested in leveraging 3D Gaussian Splatting (3DGS) to develop unified spatial languages (such as GaussianCollab) and optimizing deep learning models for vehicular networks to achieve extreme communication efficiency.

## 🔥 News
* **[May 2026]** One paper on universal any-to-any translation for collaborative perception has been accepted to **ICML 2026**!
#* **[May 2026]** Submitted our latest work on Tokenized Semantic Gaussians as a unified language for spatial intelligence to **NeurIPS 2026**.
#* **[Dec 2025]** Submitted a paper regarding point-level token optimization for collaborative perception to **CVPR 2026**.

## 🎓 Education
* **Ph.D. in Computer Science** (2023 - 2028)
    * Beijing University of Posts and Telecommunications (BUPT), Beijing, China
    * *Advisors: Jinglin Li, Quan Yuan, Guiyang Luo*
* **B.Eng. in Internet of Things Engineering** (2019 - 2023)
    * Beijing University of Technology (BJUT) / University College Dublin (UCD)

## 📝 Publications

**One Model to Translate Them All: Universal Any-to-Any Translation for Heterogeneous Collaborative Perception**
Yang Li, Weize Li, Quan Yuan, Shao Congzhang, Guiyang Luo, Yunqi Ba, **Xuanhan Zhu**, Xinyuan Ding, Xiaoyuan Fu, Jinglin Li.
*International Conference on Machine Learning (**ICML**), 2026*

[//]: # (**Tokenized Semantic Gaussians as a Unified Language for Multi-Agent Spatial Intelligence**)
[//]: # (**Xuanhan Zhu**, Yang Li, Quan Yuan, Xiyuan Sun, Zihe Ge, Xianke Wu, Guiyang Luo, Xiaoyuan Fu, Jinglin Li.)
[//]: # (*Submitted to Conference on Neural Information Processing Systems (**NeurIPS**), 2026*)

[//]: # (**Beyond BEV: Optimizing Point-Level Tokens for Collaborative Perception**)
[//]: # (Yang Li, Quan Yuan, Guiyang Luo, Xiaoyuan Fu, Yangyujia, Shao Congzhang, Rui Pan, **Xuanhan Zhu**, Jinglin Li.)
[//]: # (*Submitted to IEEE/CVF Conference on Computer Vision and Pattern Recognition (**CVPR**), 2026*)

**One is Plenty: A Polymorphic Feature Interpreter for Immutable Heterogeneous Collaborative Perception**
Yuchen Xia, Quan Yuan, Guiyang Luo, Xiaoyuan Fu, Yang Li, **Xuanhan Zhu**, Tianyou Luo, Siheng Chen, Jinglin Li.
*Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (**CVPR**), 2025*

**CollaMamba: Efficient Collaborative Perception with Cross-Agent Spatial-Temporal State Space Model**
Yang Li, Quan Yuan, Guiyang Luo, Xiaoyuan Fu, **Xuanhan Zhu**, Yangyujia, Rui Pan, Jinglin Li.
*Submitted to AAAI Conference on Artificial Intelligence (**AAAI**), 2025 (arXiv preprint)*
